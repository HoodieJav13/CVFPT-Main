...
export function BrandBackdrop({ variant = 'dashboard', photoSlot, intensity: intensityOverride, className }) {
  ...
  return (
    <div
      ...
      data-signature-treatment={intensity === 'spectacle' ? 'poster' : 'legacy'}
...
export function DashboardHero({ title, subtitle, action, testId }) {
  return (
    <section
      className="signature-surface relative isolate mb-5 min-h-[148px] overflow-hidden rounded-3xl border border-primary/25 bg-card/45 px-5 py-5 shadow-[var(--app-elev-soft)] sm:min-h-[160px] sm:px-6 sm:py-6"
      data-testid={testId}
    >
      ...
      <div className="relative z-10 flex min-h-[106px] items-end justify-between gap-4 sm:min-h-[110px]">
        <div>
          <div className="signature-primary-fill mb-2 h-[3px] w-9 rounded-full" aria-hidden />
          <h1 className="font-display text-3xl font-semibold tracking-tight sm:text-4xl lg:text-5xl">{title}</h1>
          {subtitle ? <p className="mt-1 text-sm text-[hsl(var(--signature-foreground)/0.78)]">{subtitle}</p> : null}
        </div>
