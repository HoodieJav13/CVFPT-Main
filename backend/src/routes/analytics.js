  ...
  try {
    ...
    if (!clientIds.length) {
      return res.json({
        ...
        previous: { sessions: emptyTotals, adherence: { assigned: 0, completed: 0, rate: null } },
    ...
    const [
      ...
      fetchAllRows((lo, hi) => supabaseAdmin.from('sessions')
        ...
        .order('scheduled_at', { ascending: true }).range(lo, hi)),
      fetchAllRows((lo, hi) => supabaseAdmin.from('workout_assignments')
        ...
        .order('assigned_for', { ascending: true }).range(lo, hi)),
      fetchAllRows((lo, hi) => supabaseAdmin.from('check_ins')
        ...
        .order('check_in_date', { ascending: true }).range(lo, hi)),
      ...
      fetchAllRows((lo, hi) => supabaseAdmin.from('messages')
        ...
        .eq('coach_id', coachId).eq('archived', false)
        .order('created_at', { ascending: true }).range(lo, hi)),
      ...
      fetchAllRows((lo, hi) => supabaseAdmin.from('sessions')
        ...
        .order('scheduled_at', { ascending: false }).range(lo, hi)),
    ]);
    ...
    if (assignmentIds.length) {
      const logs = await fetchAllRows((lo, hi) => supabaseAdmin.from('workout_logs')
        ...
        .order('dated_workout_assignment_id', { ascending: true }).range(lo, hi));
    ...
    if (scoredMetrics.length) {
      ...
      const entries = await fetchAllRows((lo, hi) => supabaseAdmin.from('metric_entries')
        ...
        .order('recorded_on', { ascending: true }).range(lo, hi));
    ...
    const inWindow = (lo, hi) => assignmentsAll.rows
      .filter((a) => a.assigned_for && a.assigned_for >= lo && a.assigned_for <= hi);
    ...
    const attentionAdherence = adherence(inWindow(thirtyDaysAgoDenver, todayDenver), completedAssignmentIds, todayDenver);
