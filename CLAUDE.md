...
- Hosted development schema: **current** — five versioned migrations applied to PostgreSQL 17; 23/23 tables use RLS with no policies, eight RPCs are service-role-only invoker functions, and direct anon/authenticated table grants are revoked. Program storage and transactional save/import accept one to five days.
...
- Real-auth coach/client verification: **complete except for the legal-text gate** in `docs/hardening/phase-3-4-flow-verification.md`. The expanded API matrix passes 80/80, backend regressions pass 38/38, preview browser regressions pass 4/4, and the real-auth browser suite passes 6/6 across auth-negative, recoverable load-error, client, coach/Training Builder (including deterministic paste import), session/payment/progress, and admin flows. Waiver signing awaits approved legal text. AI-assisted PDF parsing is explicitly deferred; Stripe live activation and My PT Hub migration remain outside this goal.
