...
- Hosted development schema: **six migrations applied to PostgreSQL 17; two pending on this branch** — `20260714004350_add_metric_improvement_direction.sql` and `20260714040224_stripe_subscriptions_offline_payments_and_credit_reviews.sql` are the seventh and eighth versioned migrations and must be applied before deploying their corresponding features. The applied schema has 26/26 tables using RLS with no policies, eight RPCs as service-role-only invoker functions, and revoked direct anon/authenticated table grants. The pending payment migration adds four equally locked-down tables and seven service-role-only invoker RPCs. Program storage accepts one to five days and the private `resource-library` bucket is PDF-only with a 10 MB limit.
- Payments and session entitlements: **implemented in Stripe test mode, pending hosted migration and dashboard configuration** — recurring memberships grant rolling credits only from paid initial/cycle invoices, one-time packs use hosted Checkout, cash payments receive internal receipt numbers, and courtesy/barter credits are distinct non-revenue ledger events. Coach courtesy grants above 10 credits require admin approval; refunds/disputes reverse the FIFO-attributed unused portion and flag consumed credits for manual admin review. Stripe Products/Prices, webhook event selection, and Customer Portal plan-change settings remain manual owner dashboard configuration.
...
- Real-auth coach/client verification: **complete for the accepted scope** in `docs/hardening/phase-3-4-flow-verification.md`. The expanded API matrix passes 88/88, backend regressions pass 56/56, preview browser regressions pass 6/6, and the real-auth browser suite passes 6/6 across auth-negative, recoverable load-error, client, coach/Training Builder/Resource Library, session/payment/progress, and admin flows. On 2026-07-11 the owner explicitly deferred successful waiver signing/paper-sign verification until business-approved legal text exists; no legal content was invented or changed. AI-assisted PDF parsing is also explicitly deferred; Stripe live activation and My PT Hub migration remain outside this goal.
