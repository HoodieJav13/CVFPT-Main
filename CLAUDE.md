...
- Training Builder import/export (deterministic pasted text, CSV/PDF import, `commit_program_import` transactional RPC, branded PDF export): **done for the approved deterministic scope**. Paste import uses the same Program Draft review/edit and atomic commit path, supports one to five days, reuses normalized exercise matches, and tags new exercises `source='manual'` with `review_status='needs_review'`. Close-but-not-identical active-library names now require an explicit coach choice to reuse the suggested exercise or create a new entry; no fuzzy match is auto-merged. CSV/PDF draft behavior remains three to five days. AI-assisted PDF parsing is explicitly deferred and remains safely disabled without OpenAI preview configuration.
- Resource Library: **done** — coaches/admins globally manage private-bucket PDF handouts, categories, public visibility, and soft-state client assignments; clients can list/download public or actively assigned resources through 60-second signed URLs. Archiving an assigned resource requires an explicit keep-access or revoke-access decision; keeping access preserves existing assignments while preventing new ones. Storage paths remain backend-only, uploads require PDF MIME plus signature and are capped/rate-limited at 10 MB/10 per 15 minutes, and preview mode contains deterministic public/assigned fixtures.
- Hosted development schema: **eight migrations applied and verified on PostgreSQL 17; four branch migrations pending** — the hosted schema includes metric-direction and the base Stripe/credits lifecycle with 30/30 tables using RLS with no policies and revoked direct anon/authenticated table grants. The local 12-migration schema has 16 service-role-only invoker RPCs. `20260714210610_snapshot_subscription_entitlements.sql`, `20260714220803_audited_payment_review_adjustments.sql`, `20260715005452_archive_resource_with_assignment_choice.sql`, and `20260715005846_honor_import_exercise_choices.sql` are migrations nine through twelve and must be applied before deploying this branch. Program storage accepts one to five days and the private `resource-library` bucket is PDF-only with a 10 MB limit.
- Payments and session entitlements: **implemented in Stripe test mode; corrective migrations and owner dashboard configuration remain pending** — recurring memberships snapshot their sold credit entitlement and grant rolling credits only from paid initial/cycle invoices, one-time packs use hosted Checkout, cash payments receive internal receipt numbers, and courtesy/barter credits are distinct non-revenue ledger events. Coach courtesy grants above 10 credits require admin approval; refunds/disputes reverse the FIFO-attributed unused portion and manual review resolutions apply an explicit, bounded, audited credit adjustment. Stripe Products/Prices, webhook event selection, and Customer Portal plan-change settings remain manual owner dashboard configuration.
...
- Real-auth coach/client verification: **complete for the accepted scope** in `docs/hardening/phase-3-4-flow-verification.md`. The expanded API matrix passes 88/88, backend regressions pass 62/62, preview browser regressions pass 7/7, and the real-auth browser suite passes 6/6 across auth-negative, recoverable load-error, client, coach/Training Builder/Resource Library, session/payment/progress, and admin flows. On 2026-07-11 the owner explicitly deferred successful waiver signing/paper-sign verification until business-approved legal text exists; no legal content was invented or changed. AI-assisted PDF parsing is also explicitly deferred; Stripe live activation and My PT Hub migration remain outside this goal.
