...
- Hosted development schema (2026-07-22): **16 migrations applied to PostgreSQL 17** through `20260720173000_exercise_performance_history.sql`. After PR #5 merged, `20260720120000_coach_workout_responses.sql` and `20260720173000_exercise_performance_history.sql` were applied in timestamp order; the linked local/remote migration ledger is 16/16 and a second dry run reports the remote database is up to date. The applied history includes the Stripe/credit and dormant payment-era migrations already on `main`; do not edit, remove, reorder, or roll back those applied migrations. Any correction requires explicit owner approval and a new forward-only migration. See `docs/hardening/2026-07-22-pr5-hosted-release.md`.
