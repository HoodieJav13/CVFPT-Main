...
- Hosted development schema (2026-07-18): **14 migrations applied to PostgreSQL 17** through `20260718191925_retire_session_credit_deduction.sql`. The repository now contains 16 migrations; `20260720120000_coach_workout_responses.sql` and `20260720173000_exercise_performance_history.sql` are forward-only and have been replayed together locally but have not been applied to a hosted environment. The applied history includes the Stripe/credit and dormant payment-era migrations already on `main`; do not edit, remove, reorder, or roll back those applied migrations. Any correction requires explicit owner approval and a new forward-only migration.
- Coach feedback v3 (2026-07-21): **implemented and locally verified** as separate soft-state records with one response per authorized coach/admin author per completed workout, immutable author-name snapshots, client unread/read state, assignment-aware server authorization, deterministic preview fixtures, and client Programs badges/history markers. The save RPC passed clean replay plus overlapping PostgreSQL transaction coverage; existing completion notifications, completed-workout snapshots, and credit-independent behavior remain unchanged.
