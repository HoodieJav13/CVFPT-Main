...
- Hosted development schema (2026-07-17): **13 migrations applied to PostgreSQL 17** through `20260717043317_workout_tracking_notifications.sql`. The applied history includes the three Stripe/credit migrations copied onto `main` with the workout backend commit; do not edit, remove, or roll back those applied migrations. Any reconciliation requires explicit owner approval and a new forward-only migration.
- Stripe/credit system (owner + partners decision, 2026-07-17): PR #3 (`codex/stripe-payments-and-credits`) was fully built, reviewed, and verified correct across multiple rounds, but deliberately remains unmerged. Clients continue paying outside the app, with coaches assigning access after payment is handled separately. Do not resume, extend, or build on PR #3, including its Stripe-related `previewMode.js` mocks, without an explicit decision to revisit it. Programs and Resources have never depended on credits or payment status and require no change.
- Dormant payment-code reconciliation note (2026-07-17): `main` contains a mixed, unreconciled state. The workout backend commit accidentally carried over `stripe_subscriptions_offline_payments_and_credit_reviews`, `snapshot_subscription_entitlements`, `audited_payment_review_adjustments`, `archive_resource_with_assignment_choice`, and `honor_import_exercise_choices`; their SQL content matches PR #3 after blank-line normalization. `main` also retains the pre-existing baseline `backend/src/routes/payments.js`, Stripe npm dependency, and `frontend/src/pages/client/Packages.jsx`, but history confirms those application artifacts predate PR #3 and were not added by the workout carryover; `backend/src/services/stripeCatalog.js` is absent. The current `payments.js` and `backend/src/validation/business.js` are byte-identical to the pre-PR shared base, so they predate PR #3's subscription-entitlement snapshot sourcing, audited credit-adjustment resolution, and recurring-package validation fixes even though the corrective snapshot/review migrations are applied hosted. Purchase actions remain UI-disabled when payments are unconfigured, and the backend boots without Stripe credentials, so this is inert technical debt rather than a live payment path. Do not reconcile, remove, or clean up this dormant code or schema without an explicit decision; removal requires dependency review, backup, and a new forward-only migration rather than ad hoc deletion.
